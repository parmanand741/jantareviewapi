...
final class AdminHandlers
{
    ...
    {
        Admin::require($req->payload['adminKey'] ?? null);

        $reviews = [];
        foreach (Sheets::read(Sheets::sheetName('reviews'), 'A2:R') as $r) {
            ...
            $reviews[] = [
                ...
                'store' => self::detectStoreAdmin($host),
    ...
    public static function deleteReview(Request $req): never
    {
        ...
        foreach ($all as $r) {
            ...
            if ($isTarget || $isChild) $toDelete[] = $r;
            else $remaining[] = $r;
        }
        ...
        // Write the remaining back
        Sheets::clear($revSheet, 'A2:R');
        if ($remaining) {
            $end = 1 + count($remaining);
            Sheets::write($revSheet, "A2:R{$end}", $remaining);
        }
        ...
        foreach ($toDelete as $r) {
            Audit::log([
                ...
                'productUrl'  => (string)($r[2] ?? ''),
                'stars'       => (int)($r[4] ?? 0),
                'reviewText'  => mb_substr(Crypto::decrypt((string)($r[5] ?? '')), 0, 240),
                'reportCount' => (int)($r[10] ?? 0),
                'ruleOrRestore' => $rule,
                'actionType'  => 'deleted',
                'actor'       => 'admin',
        ...
        foreach ($data as $i => $r) {
            if (($r[0] ?? '') === $caseId) {
                ...
                Audit::log([
                    ...
                    'actor' => 'admin',
                    'reason' => "Case {$caseId} acknowledged",
                    'relatedId' => $caseId,
                ]);
                Response::ok(['message' => 'Grievance acknowledged.']);
            }
        }
    ...
    private static function detectStoreAdmin(string $host): ?string
    {
        // Same mapping as PublicHandlers — kept private here for clarity.
        return $host === '' ? null : (function () use ($host) {
            $stores = [
                'amazon.' => 'Amazon',
                'amzn.' => 'Amazon',
                'flipkart.' => 'Flipkart',
            ...
            $h = strtolower($host);
            foreach ($stores as $needle => $name) if (str_contains($h, $needle)) return $name;
            return null;
        })();
    }
